...
document.addEventListener('turbolinks:load', () => {
  const sport = new Vue({
    ...
    methods: {
      submit: function(){
        ...
        if (this.member_status===true && this.premessage !== this.message){
          axios.get('http://controlfit.online/search_sport.json',{
      ...
      sport_record(idx){
        ...
        var self =this;
        axios.post("http://controlfit.online/exercise_records",sport_hash)
             .then(function(response){
      ...
      delete_record(index){
        ...
        axios.delete(`http://controlfit.online/exercise_records/${this.daily_sport[index].id}`, delete_id)
             .then(function(response){
             })
      ...
      update_record(index){
        if (Number(this.daily_sport[index].min)>=1){
          ...
          axios.patch(`http://controlfit.online/exercise_records/${this.daily_sport[index].id}`, update_daily)
               .then(function(response){
               })
        } else{
          alert("分鐘數必須要大於1 min")
    ...
    created() {
      ...
      // User information
      var self =this;
      axios.get('http://controlfit.online/blogs/new.json')
           .then(function(response){
            if (response.data.member_exist === true){
              ...
              if (self.user_id !== 0 ){
                  axios.get('http://controlfit.online/search_sport.json',{params:{ member_id: self.user_id}})
                       .then(function(response){
                  ...
                  axios.get(`http://controlfit.online/blogs/${self.user_id}/secret`)
                       .then(function(response){
    ...
    methods: {
      lookfor: function(){
        if (this.member_status === true){
            ...
            if (physical_validate === true) {
              var self = this;
              axios.post('http://controlfit.online/blogs', physical_hash)
              .then(function(response){
      ...
      update: function(){
        ...
        if (this.member_status === true){
          if(update_physical_validate === true){
             axios.patch(`http://controlfit.online/blogs/${this.user_info.user_id}`, update_physical_data)
                  .then(function(response){
      ...
      add_tdee: function(){
        let intention = { eat  : this.eatintention,
                          ...
                          tdee : this.TDEE}
        if (this.member_status === true){
             axios.patch(`http://controlfit.online/blogs/${this.user_info.user_id}/tdee`, intention)
                  .then(function(response){})                     
        }else{}
      },
    },
      ...
      axios.get('http://controlfit.online/blogs/new.json')
           .then(function(response){
            if (response.data.member_exist === true){
                 ...
                 if (self.user_id !== 0 ){
                    // day
                      axios.get('http://controlfit.online/member/foodday')
                           .then(function(response){
                                })
                           .catch((error) => {})
                      axios.get('http://controlfit.online/member/sportday')
                           .then(function(response){
                            console.log(response)
                            })
                           .catch((error) => {})
                   //week
                      axios.get('http://controlfit.online/member/foodweek')
                           .then(function(response){
                                 console.log(response)
                                })
                           .catch((error) => {})
                      axios.get('http://controlfit.online/member/sportweek')
                           .then(function(response){
                                console.log(response)
                              })
                           .catch((error) => {})
                   //month
                      axios.get('http://controlfit.online/member/foodmonth')
                           .then(function(response){
                                 console.log(response)
                                })
                           .catch((error) => {})  
                      axios.get('http://controlfit.online/member/sportmonth')
                           .then(function(response){
                            console.log(response)
                                })
                           .catch((error) => {})  
